...
export default function VisitaForm({ visita, profile, secoes, respostasIniciais, fotosIniciais, nextbittAtivo }: {
  ...
  const [modalCamposFalha, setModalCamposFalha] = useState<string[] | null>(null)
  ...
  async function handleConcluir() {
    ...
    if (camposFalha.length > 0) {
      setModalCamposFalha(camposFalha)
      return
    }
  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl pb-24 lg:pb-8">
      ...
      {/* Modal campos obrigatórios em falta */}
      {modalCamposFalha && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Campos obrigatórios em falta</h3>
                <p className="text-xs text-gray-500 mt-0.5">Preenche os seguintes campos para concluir</p>
              </div>
            </div>
            <div className="px-6 py-4 max-h-64 overflow-y-auto">
              <ul className="space-y-2">
                {modalCamposFalha.map((nome, i) => (
            ...
            <div className="px-6 py-4 bg-gray-50 rounded-b-2xl">
              <button onClick={() => setModalCamposFalha(null)}
                className="w-full py-2 rounded-lg text-sm font-medium text-white bg-[#D41317] hover:bg-red-700 transition-colors">
                OK, vou preencher
              </button>
            </div>
          </div>
                        ...
                        {dadosNextbitt.checklist.map((item: any, i: number) => (
                          <div key={i} className="px-4 py-2.5 flex items-start gap-3 bg-white hover:bg-gray-50 transition-colors">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              {item.notas && <p className="text-xs text-gray-400 mt-0.5 leading-relaxed"><span className="text-gray-300">Notas:</span> {item.notas}</p>}
                            </div>
